...
async function uploadImage(event, cardId) {
  const file = event.target.files[0];
  if (!file) return;
  event.stopPropagation();

  const statusEl = document.getElementById(`img-status-${cardId}`);
  ...
  try {
    ...
    if (!targetCard) throw new Error('找不到卡片');
    if (!targetCard.images) targetCard.images = [];
    targetCard.images.push(imageUrl);

    // 3. 存回 GitHub
    await saveImagesToGitHub(targetCard);

    // 4. 更新畫面
    const container = document.getElementById(`images-${cardId}`);
    const idx = targetCard.images.length - 1;
    const div = document.createElement('div');
    div.className = 'card-image-item';
    div.innerHTML = `
      <img src="${imageUrl}" alt="補充圖片" onclick="viewImage('${imageUrl}')">
      <button class="img-delete-btn" onclick="deleteImage(event, '${cardId}', ${idx})">✕</button>
    `;
    container.appendChild(div);

    statusEl.textContent = '✅ 圖片已儲存！';
...
async function deleteImage(event, cardId, index) {
  event.stopPropagation();
  ...
  if (!targetCard || !targetCard.images) return;

  targetCard.images.splice(index, 1);
  await saveImagesToGitHub(targetCard);

  // 重新渲染圖片區塊
  const container = document.getElementById(`images-${cardId}`);
  container.innerHTML = targetCard.images.map((url, i) => `
    <div class="card-image-item">
      <img src="${url}" alt="補充圖片" onclick="viewImage('${url}')">
      <button class="img-delete-btn" onclick="deleteImage(event, '${cardId}', ${i})">✕</button>
    </div>
  `).join('');
}
